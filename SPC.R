...
ui <- navbarPage("SPC Dashboard",
    ...
    .navbar-default .navbar-nav > li > a {
      color: #2c3e50 !important; /* Text color */
      font-weight: bold !important; /* Bold text */
      font-size: 16px !important; /* Font size */
    }
    .navbar-default .navbar-brand {
      color: #2c3e50 !important; /* Brand text color */
      font-weight: bold !important; /* Bold brand text */
      font-size: 18px !important; /* Larger brand text */
    }
    .main-container {
      margin-top: 50px;
    }
...
server <- function(input, output, session) {
  ...
  chartPlot <- reactive({
    ...
    if (input$chart_type == "Levey-Jennings") {
      ...
      qcc(x, type = "xbar.one", title = "IMR Chart", nsigmas = as.numeric(input$sigma))
    } else if (input$chart_type == "X-bar R") {
      req(input$group)
      values <- matrix(unlist(data[input$var]), ncol = input$group_size, byrow = TRUE)
      qcc(values, type = "xbar", title = "X-bar R Chart", nsigmas = as.numeric(input$sigma))
      ...
      qcc(df$Defectives, type = "p", size = df$n, title = "p Chart", nsigmas = as.numeric(input$sigma))
    } else if (input$chart_type == "c Chart") {
      qcc(x, type = "c", title = "c Chart", , nsigmas = as.numeric(input$sigma))
